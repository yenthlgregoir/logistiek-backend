...
async function validateDates() {
  ...
  const begin = update.beginDatum ?? update.$set?.beginDatum ?? this.beginDatum;

  const eind = update.eindDatum ?? update.$set?.eindDatum ?? this.eindDatum;
...
async function checkToestelRequired() {
  ...
  const nextStatus = update.status ?? update.$set?.status ?? this.status;

  if (["Leveren", "Geleverd" , "Opgehaald"].includes(nextStatus)) {
    const toestelId = update.toestel ?? update.$set?.toestel ?? this.toestel;
