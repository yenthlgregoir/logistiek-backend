...
import { Klant } from "./klant.model.js";
...
export const klantenService = {
  async getAll({ search, sort = "naam", order = "asc" }) {
    ...
    return await Klant.find(query).sort({ [sort]: order === "desc" ? -1 : 1 });
  },
  ...
  async updateLeverAdres(req, res) {
    try {
      ...
      if (!adres || !adres._id) {
        return res
          .status(400)
          .json({ message: "Leveradres met _id is verplicht." });
      }
      ...
      if (!bestaandAdres) {
        return res
          .status(400)
          .json({ message: "Dit leveradres hoort niet bij deze klant." });
      }
      ...
      res
        .status(200)
        .json({
          message: "Leveradres succesvol bijgewerkt.",
          adres: bestaandAdres,
        });
    } catch (error) {
