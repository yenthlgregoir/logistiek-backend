...
router.get('/entiteiten' , auth("logistics" , "admin") , async(req,res) => {
    try{
        const entiteiten = await ProjectLeiderService.getEntiteiten();
...
router.post('/entiteiten' , auth('logistics' , 'admin'),  async(req, res) => {
    try {
        const nieuweEntiteit = await ProjectLeiderService.createEniteit(req.body);
